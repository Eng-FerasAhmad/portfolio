...
import {ItemContentWrapper, ItemWrapper, TechstackItemContainer, ItemNameWrapper, ExpertLabelWrapper} from './styles';
...
export default function TechstackItem({category}: Props): JSX.Element {
    return (
        <TechstackItemContainer>
            ...
            {category && category.skills && category.skills.map((ts) => {
                return (
                    <ItemWrapper key={ts.id}>
                        <ItemContentWrapper>
                            <ItemNameWrapper>
                                <div>{ts.name}</div>
                                <div><IconSelector iconName={ts.icon} color={ts.color} /></div>
                            </ItemNameWrapper>
