...
export default function useLocalStorage<T>(): UseLocalStorage<T> {
  ...
  const setItemToLocalStorage = useCallback(<U>(key: LocalStorageKey, value: U) => {
  ...
  const getItemToLocalStorage = useCallback(<U>(key: LocalStorageKey): U | undefined => {
    const item: U = localStorage.getItem ? JSON.parse(localStorage.getItem(key)!) : undefined;
    return item;
  }, []);
